...
本项目受 [MulesGaming/brave-debullshitinator](https://github.com/MulesGaming/brave-debullshitinator) 启发, 但重写成了一个具有原生风格的 Windows 应用, 带有一键模式、备份, 以及更符合 Windows 用户习惯的操作流程。
...
**v2.0 新增:** 全新的界面。应用现在的外观和操作方式与 Windows 11 应用一致: 侧边导航、带开关的设置卡片、跟随 Windows 深浅色模式的主题, 并且在读取或写入系统时窗口始终保持响应。界面可翻译 (内置简体中文); 如果你想添加自己的语言, 请看 [TRANSLATING.md](TRANSLATING.md), 只需要一个 JSON 文件, 不用碰 PowerShell 代码。
...
**4. 应用会打开“主页”, 并读取本机已有的配置。** 读取在后台进行, 所以窗口一打开就能使用; 一两秒内每个开关都会显示当前状态, 底部栏会告诉你是否还有尚未应用的更改。按 `F5`, 或在“主页”或“设置”中点击“读取当前状态”, 可以重新读取。
...
| **原厂 / 不启用** | 关闭所有开关。之后点击“应用到 Brave”即可还原为默认 Brave。 |

选择模式只会改变开关, 在点击应用之前不会写入任何内容。

**6. (可选) 微调。** 每个策略分类、“系统 (任务 / 服务)”、“Hosts 屏蔽列表 (DNS 层)”和“搜索与启动”在侧边导航中都有自己的页面, 并显示已打开的数量。点击设置卡片的任意位置即可切换。底部栏会实时统计尚未应用的更改数量。

还有一个“内置 Scriptlet (高级)”页面。那是一个独立的可选工具, 用于查看 Brave 内置的广告拦截 scriptlet 规则并手动禁用其中一部分。预设和“应用到 Brave”这个大按钮从不碰它。

**7. 在应用前先点击底部栏的“预览更改”。** 它会先打开“将会发生什么”: 用通俗的语言按作用分组列出更改 (移除的功能、发送的数据更少、更快更轻量等), 并对副作用给出提醒, 例如 Brave 不再自动更新或同步被关闭。“技术细节”中列出将要新增、修改或清除的确切注册表值。预览本身不写入任何东西。

**8. 点击“应用到 Brave”** (右下角)。然后**完全关闭并重新打开 Brave**: 正在运行的标签页需要重启才能读取新策略。

**8b. (可选) 查找设置。** 在侧边导航顶部的“查找设置”中输入, 或按 `Ctrl+F`。它会一次性搜索所有页面上的策略、任务、服务和 hosts 分组, 按名称、说明、分类或域名匹配, 例如 `密码`、`遥测`、`BraveVPNDisabled`。匹配项会按页面分组显示为一个列表, 而且它们就是真实的设置: 在这里切换, 它所在的页面也会同步改变。打开 **仅显示已选** 可以只看已打开的项; 选好一个模式后, 在“主页”点击 **查看已选项**, 就能只审阅该预设即将强制生效的内容。按 `Esc` 清除搜索。“搜索与启动”和“内置 Scriptlet”不在搜索范围内; Scriptlet 页面有自己的搜索框, 专为数万条规则设计。

**9. (建议) 打开“设置”并点击“校验”。** 它会回读注册表, 确认你的选择确实生效了。你可以复制或保存这份报告。也可以打开 `brave://policy`, 检查每条策略是否显示 `Source: Platform`、`Scope: Machine`、`Status: OK`。

### 切换语言

打开“设置”并选择 **语言**。切换是实时的, 不需要重启, 并会记录在 `%LOCALAPPDATA%\Brave-Free-Origin\settings.json` 中。
...
如果你从不改动这个选项, 应用会跟随 Windows 显示语言。解析顺序是: `-Lang` → 已保存的偏好 → Windows 界面语言 → 同语言的语言文件 → 英文。

“同语言”这一步不会跨书写系统匹配: `zh-CN`、`zh-SG`、`zh-Hans-*` 会得到简体中文; 而 `zh-TW`、`zh-HK`、`zh-MO`、`zh-Hant-*` 会保持 **英文**, 除非确实安装了繁体中文语言文件, 因为简体文本并不是可用的替代品。你始终可以在“设置”中手动选择任何已安装的语言。

诊断类输出刻意保持英文: 活动日志、**预览更改** 报告和 **校验** 报告。这样即使在中文界面下使用, 提交的问题报告维护者也能看懂。
...
默认情况下, 应用会跟随 Windows 的深色或浅色应用模式 (在 Windows 11 上连标题栏也会一起跟随)。强调色始终使用 Windows 默认的蓝色。在“设置” > “主题”中可以固定为浅色或深色, 该选择同样记录在 `settings.json` 中。
...
由于本工具会在 `HKLM\Software\Policies\BraveSoftware\Brave` 下写入真实的企业策略, 只要还有策略生效, Brave 就会在菜单和 `brave://management` 中显示 **“由贵组织管理”**。这是 Chromium 的透明度机制: 任何存在计算机级策略的浏览器都会显示它。**没有**受支持的办法既保留策略又隐藏这条提示 —— Brave 拒绝提供这样的开关, 强行关闭意味着使用可能破坏策略系统的非受支持手段。唯一干净的移除方式就是移除策略 (关闭全部开关后应用, 或使用内置的还原功能)。这是设计如此, 不是缺陷。
...
- 开启 QUIC / HTTP3
- 硬件加速: 在开关旁的选择框中选择启用 (1) 或禁用 (0) (显卡驱动有问题或画面异常时, 禁用会有帮助)
- 开启内存节省
- 更轻量的启动行为
- 性能模式下使用空白主页 / 空白新标签页
- 限制磁盘缓存上限
...
- hosts 文件修改是显式的: 写入前会在界面中列出, 使用清晰标注的哨兵块 (管理员用记事本就能查看或移除), 并且可以在同一个页面中撤销。写入采用 Windows 期望的 ASCII 编码, 而不是 UTF-16。
...
或者双击某个 `.reg` 备份文件恢复之前的注册表状态; 或者用 Hosts 页面上的“移除 hosts 屏蔽块”按钮只清除 DNS 层屏蔽。
...
应用会在“设置”页面的语言选项下方显示“社区翻译, 未经审核”的小字提示。
...
Brave Free Origin 仅支持 Windows: 它是一个写入 `HKEY_LOCAL_MACHINE\Software\Policies\BraveSoftware\Brave` 的 WPF 图形界面程序。
