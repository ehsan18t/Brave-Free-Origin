...
**v2.0 新增:** 全新的界面。应用现在的外观和操作方式与 Windows 11 应用一致: 侧边导航、带开关的设置卡片、跟随 Windows 深浅色模式和强调色的主题, 并且在读取或写入系统时窗口始终保持响应。界面可翻译 (内置简体中文); 如果你想添加自己的语言, 请看 [TRANSLATING.md](TRANSLATING.md), 只需要一个 JSON 文件, 不用碰 PowerShell 代码。
...
默认情况下, 应用会跟随 Windows 的深色或浅色应用模式以及强调色 (在 Windows 11 上连标题栏也会一起跟随)。在“设置” > “主题”中可以固定为浅色或深色, 该选择同样记录在 `settings.json` 中。
